...
# 리간드 텍스트 파일을 한 줄씩 읽어서 mol2 파일로 변환하고, 단백질과 각각의 리간드 도킹 후 pdb파일로 출력
    ...
    with open("{}".format(ligand_list),'r') as f:
        while True:
            ...
            os.system(command2)
            docking_list.append('{}_docking_with_{}.pdb'.format(protein_name,ligand_name))
            with open ("{}_docking_with_{}.pdb".format(protein_name,ligand_name), 'a') as fi:
                fi.write("REMARK SMILES {}".format(smiles))

    with open("docking_list.txt",'w') as f:
        for i in docking_list:
            f.write(i)
            f.write('\n')
